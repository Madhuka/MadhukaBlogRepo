function dbqury(tableName, arg) {
	...
	sa4 += '`student_modified_time` = CURRENT_TIMESTAMP';
	console.log(s1);
	console.log(sa1);
	console.log(sa2);
	console.log(sa3);
	console.log(sa4);
	fs3 += buildf3(tableName, sa1, sa2, sa3);
	fs4 += buildf4(tableName, sa1, sa4);
	outStr += '\nvar db_query_' + tableName + '_view = "SELECT * FROM  `' + tableName + '` LIMIT 0 , 30"';
	outStr += '\nvar db_query_' + tableName + '_create = "CREATE TABLE  `' + tableName + '` (' + s1 + ',' + sf + ')";';
